' Projet Taux de conformite :: ventilation tableau croise
' -------------------------------------------------------
...
Dim ligne
Dim nPer
Dim nom, prenom, cr, region, elt, nomAgent, agent, tel1, courriel1
...
sCnxInfos= 					"Provider=Microsoft.Jet.OLEDB.4.0;" & _
          					"Data Source=" & WshShell.CurrentDirectory & ";" & _
          					"Extended Properties=""text"""     'apres text ;HDR=YES;FMT=Delimited
sCnxInfos= 					"DSN=DWH;Uid=interro;Pwd=interro;"
...
nomFichierLog = Left(nomFichierLog, i) & "_" & dateIso & Right(nomFichierLog, len(nomFichierLog) - i)
' * Ouvrir le fichier log en AJOUT, la date en premiere ligne
If (oFso.FileExists(nomFichierLog)) Then
	Set oFichierLog = oFso.OpenTextFile(nomFichierLog, ForAppending)
Else
	Set oFichierLog = oFso.CreateTextFile(nomFichierLog)
End If
oFichierLog.WriteLine(Now() & " :: Debut du fichier log de " & WScript.ScriptName)

' Quel est le fichier contenant la structure ?


' Construire les documents par PdV
' --------------------------------



' Construire les documents par Region
' -----------------------------------



' Construire les documents pour la Caisse
' ---------------------------------------
...
sqlInfosPers = sqlInfosPers & "  COMETE.CIBL_IDENT_PERS_NOYAU.ID_EDS AS idPdv, " & vbCrLf
...
'WScript.Echo "sqlInfosPers=" & sqlInfosPers
...
If (nomFichierSortie = "") Then
	oFichierLog.WriteLine(Now() & " :: Pb pour determiner /FichierSortiePlusInfosPers/ (" & nomFichierEntree & ")")
	WScript.Quit
Else
...
' La 1ere ligne contient les entetes
ligne = oFichierEntree.ReadLine
oFichierSortie.WriteLine ligne & sep & "prenom" & sep & "nom" & sep & "tel1" & sep & "courriel" & sep _
& "Nom Agent" & sep & "Agent" & sep & "PdV" & sep & "Region" & sep & "CR"
...
'WScript.Echo "infos pers pour " & nPer & ":" & oRsInfos.RecordCount
			...
			elt = oRsInfos("idPdv")
			nomAgent = oRsInfos("nomAgent")
			agent = oRsInfos("idAgent")
			tel1 = oRsInfos("tel1Client")
			courriel1 = oRsInfos("courriel1Client")
			oFichierSortie.WriteLine ligne & sep & Trim(prenom) & sep & Trim(nom) & sep & tel1 & sep & courriel1 & sep _
			& nomAgent & sep & agent & sep & elt & sep & region & sep & cr
			nbrInfos = nbrInfos + 1
		Else
