...
oFichierLog.WriteLine("nPer=" & nPer)
		oRsInfos.Open	sqlInfosPers & nPer & sqlInfosPersFin, oCnxInfos, adOpenStatic, adLockOptimistic, adCmdText

' ********************************
'oFichierLog.WriteLine("Rs.status=" & oRsInfos.Status & "-*-cnx.errors=" & oCnxInfos.Errors)
for each objErr in oCnxInfos.Errors
  msgErr = ""
  msgErr = "Description:" & objErr.Description & " Help context:" & objErr.HelpContext & _
  "Help file:" & objErr.HelpFile & " Native error:" & objErr.NativeError & _
  "Error number:" & objErr.Number & " Error source:" & objErr.Source & _
  "SQL state:" & objErr.SQLState
  oFichierLog.WriteLine(msgErr)
next
' ********************************
...
'WScript.Echo "infos pers pour " & nPer & ":" & oRsInfos.RecordCount
			...
			nbrInfos = nbrInfos + 1
		Else
			oFichierLog.WriteLine(Now() & " :: Pb recup infos avec NPer=" & nPer & vbCrLf _
				 & sqlInfosPers & nPer & sqlInfosPersFin & vbCrLf _
				 & "Nbr de reponse : " & oRsInfos.RecordCount& vbCrLf )
		End if
		oRsInfos.Close
	Else
